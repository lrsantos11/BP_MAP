...
"""
function parallel_scan_l1ball(
    data::Array{Float64,1}, b::Real, numthread::Int, spa::Float64
)::Array{Float64,1}
...
"""
function parallel_filter_l1ball(
    data::Array{Float64,1}, b::Real, numthread::Int, spa::Float64
)::Array{Float64,1}
    #the length for subvectors
    width = floor(Int, length(data) / numthread)
...
"""
    l1ball_condat_p variation that accepts a sparse vector.
"""
function l1ball_condat_p(
    data::SparseVector{Float64, Int}, b::Real, numthread::Int, spa::Float64
)::AbstractVector

    active_list = parallel_filter_l1ball(data.nzval, b, numthread, spa)
    pivot = checkL(active_list, b)
    presol = l1ball_pscan(data.nzval, pivot, numthread)
    return sparsevec(data.nzind[presol.nzind], presol.nzval, data.n)
end
