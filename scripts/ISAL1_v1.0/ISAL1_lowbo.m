function dlb = ISAL1_lowbo(A,b,x,h,mode)
%function dlb = ISAL1_lowbo(A,b,x,h,mode)
% =========================================================================
%  FILE:           ISAL1_lowbo.m
...
%                  problem (P1):  min ||x||_1  s.t.  Ax=b (underdet. sys.)
%                  by generating a feasible dual point y, i.e., y s.t.
%                  ||A'*y||_inf <= 1, and evaluating the dual ob-
%                  jective function -b'y. Several constructions of y are
...
%                       1 = Dual lower bound from scaling -b to achieve
%                           dual feasibility:- b'*b/||A'*b||_inf
%                       2 = A valid dual lower bound is obtained by (ap-
%                           proximately) solving the system 
%                                       A*A'*y = -A*sign(x)
%                           and then scaling y to achieve dual feasibility.
...
%                       4 = Another variation of 2 using a a user-supplied
%                           (subgradient-related) vector h instead of sign(x).
...
%                           seems to yield best overall duality gap esti-
%                           mates using lower bounds from this function
%                       6 = A variation of 2, strongly related to HOC:
...
%
%  OUTPUT:         dlb  The (dual) lower bound for the (primal) objective
...
switch mode
    case 0, % x,h,A,b not needed (can be empty)  dlb = 0;

    case 1, % x, h not needed (can be empty).
        y = -b./norm(-A'*b,'inf'); 
        dlb = -b'*y;
    ...
    case 5, % variation of 2 inspired by CMF-style subgradient 
        ...
        y = y./norm(A'*y,'inf');
        dlb = -b'*y;
        % Seems to provide best bounds in average, therefore set as default
...
dlb = max(dlb,0);
% dlb = max(dlb,b'*b/norm(-A'*b,'inf'));  % could also use case 1

if( isnan(dlb) ) % this can happen due to numerical stability problems
    dlb = 0;
